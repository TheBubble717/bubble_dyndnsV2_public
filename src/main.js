...
async function bubbledns() {
    ...
    log.addlog(`Starting in ${env} Mode using Config: ${configFileName}`, { color: "green", warn: "Startup-Info" })


    //Activate MYSQL
    log.addlog("Activating Mysql-Connection", { color: "green", warn: "Startup-Info" })
    var dblog = new logclass({debug:config.mailclient.debug})
    ...
    await classdata.db.connect(async function (err, res) {
        if (err) {
            log.addlog(err, { color: "red", warn: "Startup-Error" });
            process.exit()
        }
        log.addlog(res, { color: "green", warn: "Startup-Info" })

    });
    await classdata.db.enable_routines()

    //Activate Mailservice
    log.addlog("Activating Mailserver-Connection", { color: "green", warn: "Startup-Info" })
    var maillog = new logclass({debug:config.mailclient.debug})
    ...
    if (existsindb.length) {
        ...
        log.addlog("Activating DNS-Server", { color: "green", warn: "Startup-Info" })
        var dnslog = new logclass({debug:config.dnsserver.debug})
        ...
        log.addlog("This BubbleDNS-Server is not found in the Database, waiting for Database-Update", { color: "yellow", warn: "Startup-Info" })
    }
    ...
    if (existsindb_enableweb.length) {
        //Activate Web-Server
        log.addlog("Activating WEB-Server", { color: "green", warn: "Startup-Info" })
        var weblog = new logclass({debug:config.webserver.debug})
    ...
    if (classdata.db.routinedata.bubbledns_settings.newServer) {
        log.addlog("First Time Installer started!", { color: "green", warn: "FirstStartup" })
        ...
        log.addlog(`Admin "${newuserdata.get_user_public().mailaddress}" created with password "${randompassword}"`, { color: "green", warn: "FirstStartup" })
        ...
        log.addlog(`Bubbledns-Server ns1.${classdata.db.routinedata.bubbledns_settings.maindomain} created and set to Main-Server`, { color: "green", warn: "FirstStartup" })
        log.addlog(`Please add a second Bubbledns-Server nsX.${classdata.db.routinedata.bubbledns_settings.maindomain} for the functionality of the server`, { color: "green", warn: "FirstStartup" })
        ...
        log.addlog(`Domain ${classdata.db.routinedata.bubbledns_settings.maindomain} created as builtin and owner set to ${newuserdata.get_user_public().mailaddress}`, { color: "green", warn: "FirstStartup" })
        ...
        log.addlog(`Disabling First Time Installer`, { color: "green", warn: "FirstStartup" })
        log.addlog(`Killing Process`, { color: "green", warn: "FirstStartup" })
        process.abort()
    }
}
